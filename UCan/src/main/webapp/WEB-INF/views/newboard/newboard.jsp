		...
		<div class="page-wrapper">
			...
			<div class="page-content">
				<nav class="page-breadcrumb">
					<ol class="breadcrumb">
						<li class="breadcrumb-item"><a href="/newboard">Tables</a></li>
                    ...
                    <table class="table table-hover">
                      <tbody>
                        ...
                        <tr><td>조회수</td><td id="readCountDetail">#</td></tr>
                         ...
                         <tr><td>반려 동물 종류</td><td id="dogNameSelectDetail">#</td></tr>                                               
                         <tr><td>위치</td><td id="placeNameDetail">#</td></tr>
            
                      </tbody>
                    </table>
                  				 
					
							     <div id="dogUrlSelectDetail">
						 </div>
 ...
 async defer   ></script>
	<script>
	  ...
	  function dogList(){
	   ...
	   /* 
	   function makeDogResultHtml(result){  
	   ...
	   function makeDogHtml(list){
		   ...
		   for(var i=0; i<list.length;i++){
			 	  ...
			 	  dogId = list[i].dogId;
			 	  dogName = list[i].dogName;
			 	  
//		     	  var imghtml = '<img class="dogImage" src = '+dogUrl+'/>';
		     	  var imghtml = 
		     		  			'<img class="dogImage" data-dogId = '+dogId+' data-dogName = '+dogName+' src = '+dogUrl+'/>';
		     	 				
				  $("#dogBody").append(imghtml);
				  console.log(dogId);
				  
			}
		   makeDogHtmlEventHandler();
		...
		function boardInsert() {
			...
			formData.append("userSeq", '<%=userDto.getUserSeq()%>');
		     ...
		     formData.append("dogId", $(".sizess").find(".dogImageSelected").attr("data-dogId"));		      
		      formData.append("file", $("#inputFileUploadInsert")[0].files[0]);
		      formData.append("placeId", $(".place").attr("data-place_id"));//=place_id
		      formData.append("placeLat", $(".place").attr("data-place_lat"));//=place_lat		     
		      formData.append("placeLng", $(".place").attr("data-place_lng"));//=place_lng
		      formData.append("placeName", place_nm); //place_nm은 전역 변수로 선언해 줬기 때문에 가능. 값은 저장해 둠.
		      formData.append("placeAddress", place_address); 
	   ...
	   function makeDogHtmlEventHandler(){
		   $(".sizess").find(".dogImage").click(function(){
			   
			   $(".sizess").find(".dogImage").removeClass("dogImageSelected");
			   
			$(this).addClass("dogImageSelected");
			
 // var dogId = $(this).attr("data-dogId");
			//  dogSelect(dogId);
			
			//var dogId = $("#dogIdModal").val("dogId");
			
			var dogId = $(this).attr("data-dogId");
			var dogName =$(this).attr("data-dogName");
		
		 	//$("#dogId").html(readCount);
		 	//$("#readCountDetail").html(readCount);
		 	
			   $("#dogSelectResult").html(dogName);
		...
		$.ajax({
				...
				error : function(jqXHR, textStatus, errorThrown) {
					if (jqXHR.responseText == "timeout") {
						...
						alertify.notify('Opps!! dogSelect 과정에 문제가 생겼습니다.', 'error', //'error','warning','message'
						3, //-1
						function() {
...
*/
			...
			$.ajax({
				...
				error : function(jqXHR, textStatus, errorThrown) {
					if (jqXHR.responseText == "timeout") {
						...
						alertify.notify('Opps!! 상세 조희 과정에 문제가 생겼습니다.', 'error', //'error','warning','message'
						3, //-1
						function() {
							//여기       console.log(jqXHR.responseText); 
							console.log(jqXHR.responseText);
		...
		function makeDetailHtml(detail) {
			...
			var savePlaceName = detail.placeName;
			var placeLat = detail.placeLat;
			var placeLng = detail.placeLng;
			var dogName = detail.dogName;
						
			temp = new google.maps.LatLng(placeLat, placeLng);
	        MAP_VIEW_DETAIL.setCenter(temp);
			...
			var contentString = savePlaceName;
			...
			MARKER2.addListener('click', function(){
           	 infowindow2.open(MAP_VIEW_DETAIL, MARKER2);
            });
			...
			$("#readCountDetail").html(readCount);
			$("#dogNameSelectDetail").html(dogName);
		//	$("#dogUrlSelectDetail").html(dogUrl);
		    $("#placeNameDetail").html(savePlaceName);
			
			
			//FileList
			$("#fileListDetail").html("");
			$("#dogUrlSelectDetail").html("");
			...
			if (fileList.length > 0) {
				for (var i = 0; i < fileList.length; i++) {
					...
					var fileImagehtml = '<img class="fileImage"  style="width:20%" src = '+fileUrl+'/>';
					...
					$("#dogUrlSelectDetail").append(fileImagehtml);
				}
			}
  ...
  function initMap(){
     ...
     MAP_VIEW_DETAIL = new google.maps.Map(document.getElementById("mapDetail"), {
         center: DEFAULT_MAP_LOCATION,
           scrollwheel: true,
           zoom: 17
      });
    ...
    // autocomplete.bindTo('bounds', MAP_VIEW_DETAIL);
  ...
  function add_place_event_handler(){
      ...
      $(".btnAddPlace").click(function(){
          ...
          place_id = $(this).closest(".place").data("place_id");
          place_lat = $(this).closest(".place").data("place_lat");
          place_lng = $(this).closest(".place").data("place_lng");
          place_nm = $(this).closest(".place").find(".place_nm").html();
          place_address = $(this).closest(".place").find(".place_address").html();
